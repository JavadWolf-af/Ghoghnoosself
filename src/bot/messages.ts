...
export const CLOCK_PANEL_MESSAGE = (enabled: boolean): string =>
  `🕐 *ساعت هوشمند*\n\n` +
  `━━━━━━━━━━━━━━━━━\n` +
  `این ابزار هر یک دقیقه یکبار، نام‌خانوادگی اکانت شما را به ساعت فعلی ایران تغییر می‌دهد.\n\n` +
  `وضعیت فعلی: ${enabled ? "🟢 *روشن*" : "🔴 *خاموش*"}\n\n` +
  `از دکمه زیر ساعت را روشن یا خاموش کنید:`;

export const CLOCK_ENABLED_MESSAGE = (): string =>
  `🟢 *ساعت فعال شد*\n\n` +
  `نام‌خانوادگی شما هر دقیقه به ساعت ایران تبدیل می‌شود.`;

export const CLOCK_DISABLED_MESSAGE = (): string =>
  `🔴 *ساعت غیرفعال شد*\n\n` +
  `نام‌خانوادگی شما دیگر تغییر نمی‌کند.`;

export const CLOCK_AUTH_PROMPT = (phone: string): string =>
  `🔐 *احراز هویت لازم است*\n\n` +
  `━━━━━━━━━━━━━━━━━\n` +
  `برای فعال‌سازی ساعت، ربات باید به اکانت شما دسترسی داشته باشد.\n\n` +
  `یک کد تأیید به شماره \`${esc(phone)}\` ارسال می‌شود.\n` +
  `لحظاتی صبر کنید...`;

export const CLOCK_AUTH_CODE_PROMPT = (): string =>
  `📲 *کد تأیید ارسال شد*\n\n` +
  `━━━━━━━━━━━━━━━━━\n` +
  `کد ۵ یا ۶ رقمی که در تلگرام دریافت کردید را وارد کنید:\n\n` +
  `↩️ برای انصراف 🔙 را بزنید`;

export const CLOCK_AUTH_CODE_INVALID = (): string =>
  `❌ *کد اشتباه است*\n\n` +
  `کد وارد شده معتبر نیست.\n` +
  `دوباره امتحان کنید یا 🔙 را بزنید.`;

export const CLOCK_AUTH_SUCCESS = (): string =>
  `✅ *احراز هویت موفق*\n\n` +
  `━━━━━━━━━━━━━━━━━\n` +
  `اتصال برقرار شد! اکنون می‌توانید ساعت را فعال کنید.`;

export const CLOCK_AUTH_2FA_MESSAGE = (): string =>
  `🔒 *رمز دو مرحله‌ای (2FA) فعال است*\n\n` +
  `━━━━━━━━━━━━━━━━━\n` +
  `متأسفانه امکان فعال‌سازی ساعت برای اکانت‌هایی که رمز دو مرحله‌ای دارند وجود ندارد.\n\n` +
  `برای استفاده از ساعت، 2FA را از تنظیمات تلگرام غیرفعال کنید.`;

export const CLOCK_AUTH_ERROR = (): string =>
  `❌ *خطا در احراز هویت*\n\n` +
  `اتصال به اکانت ناموفق بود.\n` +
  `ممکن است 2FA فعال باشد یا کد منقضی شده باشد.\n` +
  `دوباره از طریق منو امتحان کنید.`;
